...
def test_perron(serie, break_point=None, parallel=False):
    print('Test Perron')

    if break_point is None:
        # Si el breakpoint es desconocido, elegimos la función a ejecutar
        if parallel:
            dataframes = grid_perron_parallel(serie=serie)
        else:
...
def _process_breakpoint(args):
    ...
    bp, serie, model_type, input_fn = args
    
    try:
        # Calcular el p_star óptimo PARA ESTE BREAKPOINT ESPECÍFICO
        table = hannan_rissanen(serie, 
                                max_rezagos=50, 
                                verbose=False, 
                                model_type='ar', 
                                inp_fn=input_fn)
        ...
        if table.empty:
            return None # Devolver None si no se puede calcular

        p_star = table['BIC'].idxmin()
...
def grid_perron_parallel(serie, trim=0.15):
    """
    ...
    for model_type, input_fn in [('model_a', create_model_a), 
        ...
        print(f"Distribuyendo cálculo para {model_type} en {multiprocessing.cpu_count()} núcleos...")
        
        # 1. Prepara la lista de tareas (una tupla de args para cada bp)
        tasks = [(bp, serie, model_type, input_fn) for bp in grid]
